...
function findSitesQuery() {
  ...
  //send in a GET
  //get should return an array from the sql database.......How else could this be done?
  ...
  $.ajax({
    ...
    for (var i = 0; i < response.length; i++) {
      locationID = response[i].LOCATION_ID;
      locationName = response[i].LOCATION_NAME;
      locationAddress = response[i].LOCATION_ADDRESS;
      locationHours = response[i].LOCATION_HOURS;
      locationPhone = response[i].LOCATION_PHONE;
      //latLng = results[i].geometry.y + ", " + results[i].geometry.x;
      locationEmail = response[i].LOCATION_EMAIL;
      ...
      contentString = '<strong>' + locationName + '</strong><br>' +
        '<a href="https://www.google.com/maps/search/?api=1&query=' + locationAddress + '">' + locationAddress + '</a>' + '<br>' +
        'Serving on: ' + locationHours + '<br>' +
        'Call ' + '<a href="tel:+1-' + locationPhone + '">' + locationPhone + '</a>' + ' to confirm meal times</p></li>';
    ...
    for (var i = 0, length = listingArray.length; i < length; i++) {
        var address = listingArray[i].address;
        //console.log(address)
...
function addMarkers() {
  ...
  for (var i = 0, length = listingArray.length; i < length; i++) {
    //markerLabel = (i + 1);
      var address = listingArray[i].LOCATION_ADDRESS;
      geocoder.geocode( { 'address': address}, function(results, status) {
        if (status == 'OK') {
          map.setCenter(results[0].geometry.location);
          var marker = new google.maps.Marker({
          ...
          infowindow.setContent(listingArray[i].contentString);
        } else {
          alert('Geocode was not successful for the following reason: ' + status);
        }
      });
